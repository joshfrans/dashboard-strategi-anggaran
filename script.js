...
let investmentData = {
  ...
  // Nilai berikut sebelumnya ditulis langsung di index.html sehingga tidak
  // ikut berubah saat data investasi di-import. Default = nilai lama, jadi
  // template lama yang belum memuat kode ini tetap aman.
  aiOfficeTotal: "10.636,92 M",
  aiSarprasTotal: "255,31 M",
  akiOfficeTotal: "1.175,85 M",
  akiSarprasTotal: "178,66 M",
  aiAdditionalNeed: "Indikasi kebutuhan: DIV TEN 100 M, DIV RSL 80 M, DIV MEB 80 M.",
  aiCarryoverPrk: "42",
  aiCarryoverTotal: "2.912,71 M",
  aiCarryoverUsed: "1.208,99 M",
  aiCarryoverUnused: "1.703,71 M",
  sarprasSkaiIssued: "270,70 M",
  sarprasAkiIssued: "98,87 M",
  sarprasAiMurni: "Belum Terbit",
  sarprasPrkProgress: "9 selesai / 8 berjalan"
};
...
const investmentFieldLabels = {
  ...
  aiOfficeTotal: "AI Terbit Kantor Pusat",
  aiSarprasTotal: "AI Terbit Sarpras Unit",
  akiOfficeTotal: "AKI Terbit Kantor Pusat",
  akiSarprasTotal: "AKI Terbit Sarpras Unit",
  aiAdditionalNeed: "Indikasi Tambahan Kebutuhan AI",
  aiCarryoverPrk: "Jumlah PRK AI Luncuran KP",
  aiCarryoverTotal: "Total AI Luncuran KP",
  aiCarryoverUsed: "AI Luncuran KP Digunakan 2026",
  aiCarryoverUnused: "AI Luncuran KP Belum Digunakan",
  sarprasSkaiIssued: "SKAI Terbit Sarpras",
  sarprasAkiIssued: "AKI Terbit Sarpras",
  sarprasAiMurni: "Status AI Murni 2026 Sarpras",
  sarprasPrkProgress: "Progress PRK Sarpras"
};
...
function evUnitIdentity(unit) {
  return [
    String(unit?.unit || "").trim().toLowerCase(),
    numberFromImport(unit?.distance, 0),
    String(unit?.nearestSpklu || "").trim().toLowerCase(),
    numberFromImport(unit?.unitLat ?? unit?.mapY, 0),
    numberFromImport(unit?.unitLng ?? unit?.mapX, 0)
  ].join("|");
}

function dedupeEvUnits(units = []) {
  const seen = new Set();
  return units.filter((unit) => {
    const key = evUnitIdentity(unit);
    if (!unit?.unit || seen.has(key)) return false;
    seen.add(key);
    return true;
  });
}
...
// Dataset EV lengkap (assets/ev-spklu-data.js, ~2,6 MB) tidak dimuat lewat
...
function strategyDataFreshness() {
  const dataTime = (document.querySelector("[data-last-updated]")?.textContent || "").trim();
...
function setStrategySourceStatus(sourceLabel = "Google Sheets (sinkron otomatis)", syncedAt = new Date(), detail = "") {
  ...
  // Investasi/AO (DASHBOARD_FRESHNESS_SLA_DAYS). Sebelumnya snapshot 3 hari
...
function updateStrategySourceCheckedTitle() {
  const element = document.getElementById("strategySourceStatus");
  if (!element || !strategySnapshotCheckedAt) return;
  const checked = strategySnapshotCheckedAt.toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" });
  const base = (element.title || "").replace(/ · diperiksa \d{2}[.:]\d{2} WIB$/, "");
  const title = `${base} · diperiksa ${checked} WIB`;
  element.title = title;
  element.closest(".date-card")?.setAttribute("title", title);
}
...
// Dokumen resmi: Realisasi RKM DIV GA Agustus 2026 (diterbitkan 8 September 2026).
// Periode ini dipertahankan sebagai sumber otoritatif agar cache atau snapshot lama
// tidak mengembalikan Agustus ke status belum diukur.
const performanceAuthoritativePeriods = {
  "2026-08": {
    source: "Realisasi RKM DIV GA Agustus 2026",
    sourceDate: "8 September 2026",
    score: 102.60,
    status: { total: 10, green: 8, amber: 0, red: 1, gray: 1 },
    rows: [
      { no: 1, indicator: "Efisiensi Biaya", unit: "", weight: 20, target: "", targetPeriod: "", realization: "", achievement: "110%", score: "22,00", status: "Tercapai" },
      { no: "", indicator: "a. Efektifitas Biaya Administrasi Umum", unit: "Rp Miliar", weight: 14, target: "8.856,16", targetPeriod: "5.980,48", realization: "4.570,42", achievement: "110%", score: "15,40", status: "Tercapai" },
      { no: "", indicator: "b. Pengendalian NAC (Non Allowable Cost)", unit: "Rp Miliar", weight: 6, target: "848,07", targetPeriod: "565,38", realization: "383,37", achievement: "110%", score: "6,60", status: "Tercapai" },
      { no: 2, indicator: "Optimalisasi penggunaan aset tanah dan bangunan", unit: "Rp Miliar", weight: 12, target: "147,74", targetPeriod: "79,78", realization: "102,85", achievement: "110%", score: "13,20", status: "Tercapai" },
      { no: 3, indicator: "Penyediaan Dokumen Pengadaan Barang Jasa Kantor Pusat sampai dengan selesainya draft kontrak (Anggaran Operasi dan Investasi)", unit: "", weight: 12, target: "", targetPeriod: "", realization: "", achievement: "107,23%", score: "12,87", status: "Tercapai" },
      { no: "", indicator: "a. Pengadaan Langsung / Penunjukan Langsung Pasca Kualifikasi", unit: "%", weight: "", target: "93,96", targetPeriod: "93,96", realization: "97", achievement: "103%", score: "", status: "Tercapai" },
      { no: "", indicator: "b. Penunjukan Langsung Pra Kualifikasi", unit: "%", weight: "", target: "85,19", targetPeriod: "85,19", realization: "92", achievement: "108%", score: "", status: "Tercapai" },
      { no: "", indicator: "c. Tender Terbuka (Pascakualifikasi) / Tender Terbatas (DPT/Shortlist) / Seleksi Umum (Pascakualifikasi) / Seleksi Terbatas (Pascakualifikasi)", unit: "%", weight: "", target: "92,86", targetPeriod: "92,86", realization: "100", achievement: "108%", score: "", status: "Tercapai" },
      { no: "", indicator: "d. Seleksi Umum (Prakualifikasi) / Seleksi Terbatas (Prakualifikasi) / Tender Terbuka (Prakualifikasi)", unit: "%", weight: "", target: "83,33", targetPeriod: "83,33", realization: "100", achievement: "110%", score: "", status: "Tercapai" },
      { no: 4, indicator: "Integrated GA Management System", unit: "", weight: 12, target: "", targetPeriod: "", realization: "", achievement: "87,76%", score: "10,53", status: "Perlu Peningkatan" },
      { no: "", indicator: "a. Optimalisasi penyerapan anggaran pemeliharaan sarana terkonsolidasi", unit: "%", weight: "", target: "95-100", targetPeriod: "95-100", realization: "71,75", achievement: "75,53%", score: "", status: "Perlu Peningkatan" },
      { no: "", indicator: "b. Pengembangan Aplikasi Transformasi GA", unit: "Waktu", weight: "", target: "31 Desember 2026", targetPeriod: "Laporan progres pengembangan aplikasi", realization: "Laporan progres pengembangan aplikasi", achievement: "100%", score: "", status: "Tercapai" },
      { no: 5, indicator: "Penyusunan Kebijakan General Affair", unit: "Waktu", weight: 12, target: "31 Desember 2026", targetPeriod: "Pembahasan dengan Divisi Terkait", realization: "Pembahasan dengan Divisi Terkait", achievement: "100%", score: "12,00", status: "Tercapai" },
      { no: 6, indicator: "Proses Sentralisasi Pembayaran", unit: "%", weight: 10, target: "93,92", targetPeriod: "93,92", realization: "93,97", achievement: "100%", score: "10,005", status: "Tercapai" },
      { no: 7, indicator: "Manajemen Kearsipan", unit: "%", weight: 10, target: "95", targetPeriod: "95", realization: "95", achievement: "100%", score: "10,00", status: "Tercapai" },
      { no: 8, indicator: "Implementasi smart and green building", unit: "SM 1: Waktu; SM 2: Unit", weight: 6, target: "7", targetPeriod: "Audit Energi", realization: "Audit Energi", achievement: "100%", score: "6,00", status: "Tercapai" },
      { no: 9, indicator: "Usulan Anggaran Investasi Non Infrastruktur Ketenagalistrikan-Sarana Prasarana Umum untuk Tahun 2026", unit: "%", weight: 6, target: "100", targetPeriod: "100", realization: "100", achievement: "100%", score: "6,00", status: "Tercapai" },
      { no: 10, indicator: "Kepatuhan, Maturity Level dan Tata Kelola Perusahaan", unit: "", weight: "", target: "", targetPeriod: "", realization: "", achievement: "", score: "", status: "Belum dilakukan pengukuran" },
      { no: "", indicator: "a. Implementasi Maturity Level", unit: "", weight: "", target: "Maks. -2", targetPeriod: "", realization: "", achievement: "", score: "", status: "Belum dilakukan pengukuran" },
      { no: "", indicator: "b. Kepatuhan", unit: "", weight: "", target: "Maks. -3", targetPeriod: "", realization: "", achievement: "", score: "", status: "Belum dilakukan pengukuran" },
      { no: "", indicator: "c. Tata Kelola Perusahaan", unit: "", weight: "", target: "Maks. -1", targetPeriod: "", realization: "", achievement: "", score: "", status: "Belum dilakukan pengukuran" }
    ]
  }
};
...
function policyMetrics() {
  ...
  return {
    ...
    followUp: onProgress,
    exceptions: flat.filter((status) => status === "no-ratification").length
  };
}
...
function renderPolicyEntities() {
  ...
  if (!target) return;
  setSafeHtml(target, policyEntities
...
function renderPolicyTypes() {
  ...
  if (!target) return;
  setSafeHtml(target, policyTypes
...
function performanceSourceForPeriod(key = strategyPeriodKey()) {
  return performanceAuthoritativePeriods[key] || null;
}

function performanceSourceDescription(key = strategyPeriodKey()) {
  const source = performanceSourceForPeriod(key);
  return source ? `Sumber: ${source.source} · ${source.sourceDate}` : "";
}
...
function applyAuthoritativePerformancePeriods() {
  Object.entries(performanceAuthoritativePeriods).forEach(([key, source]) => {
    ...
    performanceStatusByPeriod[key] = { ...source.status };
  });
  applySelectedPerformancePeriod();
}

function achievementPercentLabel(value) {
  if (value === null || value === undefined || value === "") return "";
  const parsed = numberFromImport(value, NaN);
  if (!Number.isFinite(parsed)) return String(value);
  const percent = Math.abs(parsed) <= 2 ? parsed * 100 : parsed;
...
function performanceScoreStatus(score, summary = null) {
  ...
  if (score >= 100) {
    if (Number(summary?.gray || 0) > 0) {
      const redNote = Number(summary?.red || 0) > 0
        ? ` dan ${summary.red} indikator masih di bawah target`
        : "";
      return {
        tone: "amber",
        label: "Tercapai Sementara",
        message: `Nilai berada di atas target, tetapi ${summary.gray} indikator belum diukur${redNote}.`
      };
    }
    return {
      tone: "green",
...
function emptyPerformanceStatusSummary() {
  return {
    ...
    amber: 0,
    red: 0,
    gray: MIN_PERFORMANCE_MAIN_INDICATORS
  };
}
...
function updateStrategyPeriodLabels() {
  ...
  if (performanceScoreLabel) performanceScoreLabel.title = performanceSourceDescription();
    ...
    ? rowsForDisplay
        .map((row) => `
          <tr class="${row.no ? "performance-main-row" : "performance-sub-row"}">
            ...
            <td>${achievementPercentLabel(row.achievement)}</td>
...
function refreshOpenPerformanceDetail() {
  ...
  updateStrategyPeriodLabels();
  if (subtitle) {
    const sourceDescription = performanceSourceDescription();
    subtitle.textContent = `Menampilkan detail NKO ${strategyPeriodUntilLabel()} berdasarkan laporan pencapaian KPI Divisi Umum dan Aset Properti.${sourceDescription ? ` ${sourceDescription}.` : ""}`;
    subtitle.hidden = false;
  }
  setSafeHtml(body, renderDetailPerformance());
  if (window.lucide) window.lucide.createIcons();
}
...
function renderPolicyPrepRows() {
  ...
  if (!target) return;
  setSafeHtml(target, policyPrepData
    .map((row, index) => `
      <tr>
...
// Status tindakan selalu tampil sebagai ikon + label dengan warna status yang
// sesuai artinya. Sebelumnya CSS mewarnai seluruh kolom status hijau, jadi
// "Perlu Percepatan" dan "Perlu Pembaruan" terbaca seolah aman.
const STATUS_CHIP_TONES = {
  "baik": ["good", "circle-check"],
  "monitor": ["neutral", "eye"],
  "perlu monitoring": ["watch", "circle-alert"],
  "perlu perhatian": ["watch", "circle-alert"],
  "perlu percepatan": ["serious", "triangle-alert"],
  "perlu pembaruan": ["serious", "refresh-cw"],
  "perlu kontrol": ["serious", "triangle-alert"],
  "kritis": ["critical", "octagon-alert"]
};

function statusChip(label) {
  const text = String(label || "-");
  const [tone, icon] = STATUS_CHIP_TONES[text.trim().toLowerCase()] || ["neutral", "circle"];
  const chip = document.createElement("span");
  chip.className = `status-chip ${tone}`;
  const glyph = document.createElement("i");
  glyph.setAttribute("data-lucide", icon);
  glyph.setAttribute("aria-hidden", "true");
  chip.append(glyph, document.createTextNode(text));
...
function updateAnalyticsDashboard() {
  ...
  if (executiveMessage) {
    executiveMessage.textContent = `Laporan manajemen mengkonsolidasikan data dengan periode berbeda: Strategi ${strategyPeriodFullLabel()}, AO Korporat ${aoCorporateData.period}, AO Kantor Pusat ${aoOfficeData.period}, dan Investasi ${investmentData.reportDate}. Ratifikasi mencatat ${policy.done} selesai, ${policy.onProgress} on progress, dan ${policy.noRatification} Tidak Ratifikasi yang perlu validasi status pengecualiannya. Change Request berada pada progress ${percentLabel(crProgress)} dari ${crTotal} CR. NKO ${performanceMeasured ? `${nkoLabel(performanceScore)} (${performanceState.label})` : "belum diukur untuk periode terpilih"}. Rekomendasi lintas domain harus dibaca dengan konteks periode masing-masing.`;
  }
  ...
  if (bars) {
    setSafeHtml(bars, [
      ...
      makeAnalyticsBar(performanceMeasured ? "NKO terukur" : "NKO belum diukur", performanceMeasured ? Math.min(performanceScore, 120) : 0, performanceMeasured ? "green" : "neutral").replace(`<strong>${percentLabel(performanceMeasured ? Math.min(performanceScore, 120) : 0)}</strong>`, `<strong>${performanceMeasured ? nkoLabel(performanceScore) : "-"}</strong>`),
  ...
  if (actionRows) {
    setSafeHtml(actionRows, [
      ...
      .map((row) => `<tr><td>${row[0]}</td><td>${row[1]}</td><td>${statusChip(row[2])}</td><td>${row[3]}</td></tr>`)
      .join(""));
    if (window.lucide) window.lucide.createIcons();
  }
...
function updateAlertCenter() {
  ...
  const alerts = [
    ...
    priorityCr && {
      area: "Change Request",
      text: `${priorityCr.app} perlu tindak lanjut: progress ${percentLabel(Number(priorityCr.progress || 0))}, ${overdueDays(priorityCr.target) ? `terlambat ${overdueDays(priorityCr.target)} hari` : `target ${priorityCr.target || "belum tersedia"}`}.`,
      level: overdueDays(priorityCr.target) > 0 || Number(priorityCr.progress || 0) < 30 ? "High" : "Medium"
    },
    akiProgress < 70 && {
      area: "Investasi",
...
function updateDashboardMetrics() {
  ...
  const performanceSummary = getPerformanceStatusSummary();
  const performanceMeasured = hasTrustedPerformanceMeasurement(strategyPeriodKey(), performanceRowsForPeriodKey());
  const performanceDisplay = performanceMeasured ? nkoLabel(performanceScore) : "-";
  ...
  const values = {
    ...
    performanceScore: performanceDisplay,
    performanceIndicatorCount: performanceSummary.total,
    summaryNko: performanceDisplay,
    ...
    analyticsNko: performanceDisplay,
  ...
  if (summaryText) {
    summaryText.textContent = `Strategi & Evaluasi menunjukkan ${policy.total} status kebijakan dari ${values.summaryPolicyEntities} entitas SH/AP dan ${values.summaryPolicyTypes} jenis kebijakan, terdiri dari ${values.summaryPolicyDone} selesai, ${policy.onProgress} on progress, dan ${policy.noRatification} Tidak Ratifikasi yang perlu validasi status pengecualiannya. Monitoring kinerja ${strategyPeriodUntilLabel()} ${performanceMeasured ? `mencatat NKO ${performanceDisplay} dengan ${performanceSummary.green} indikator tercapai, ${performanceSummary.amber} hampir tercapai${performancePendingText}` : "belum memiliki pengukuran NKO lengkap"}. Pada transformasi aplikasi terdapat ${total} Change Request dengan progress keseluruhan ${progressLabel}, terdiri dari ${done} selesai, ${onProgress} on progress, dan ${notStarted} belum mulai. Penyusunan kebijakan layanan GA memonitor ${prepTotal} kebijakan, serta implementasi PLN Business Excellence menunjukkan realisasi ${smartLabel(businessScore, businessScore <= 1 ? "percent" : "plain")} pada semester utama.`;
  }
...
async function ensureXlsxLibrary() {
  try {
    ...
    console.info(error);
    alert("Library Excel lokal belum dapat dimuat. Muat ulang dashboard, lalu coba kembali atau gunakan export CSV/JSON.");
    return false;
  }
}
...
// Nilai rupiah dalam juta ("M"); "T" dikonversi ke juta agar dua nilai
// dengan satuan berbeda tetap bisa dibandingkan.
function investmentAmountInMillion(value) {
  const amount = numberFromImport(value, NaN);
  if (!Number.isFinite(amount)) return NaN;
  return /\bT\b/i.test(String(value)) ? amount * 1000 : amount;
}

function investmentSharePct(part, other) {
  const a = investmentAmountInMillion(part);
  const b = investmentAmountInMillion(other);
  if (!Number.isFinite(a) || !Number.isFinite(b) || a + b <= 0) return "";
  return `${((a / (a + b)) * 100).toLocaleString("id-ID", { maximumFractionDigits: 1 })}%`;
}

function updateInvestmentDashboard() {
  // Porsi AI per lokasi dihitung dari nilai terbit, bukan ditulis tangan,
  // supaya dua batang komposisi selalu memakai ukuran yang sama.
  investmentData.aiOfficeSharePct = investmentSharePct(investmentData.aiOfficeTotal, investmentData.aiSarprasTotal);
  investmentData.aiSarprasSharePct = investmentSharePct(investmentData.aiSarprasTotal, investmentData.aiOfficeTotal);
  investmentData.akiOfficeSharePct = investmentSharePct(investmentData.akiOfficeTotal, investmentData.akiSarprasTotal);
  investmentData.akiSarprasSharePct = investmentSharePct(investmentData.akiSarprasTotal, investmentData.akiOfficeTotal);
...
function renderDetailPerformance() {
  ...
  const scoreLabel = scoreMeasured ? nkoLabel(score) : "-";
  ...
  const nextFocus = !scoreMeasured
    ...
    : score >= 100 && !summary.gray
      ...
      : score >= 100
        ? ["Lengkapi & Pulihkan", `Tuntaskan pengukuran ${summary.gray} indikator dan pulihkan ${summary.red} indikator di bawah target sebelum menetapkan capaian final.`]
...
// Tinggi batang harus sebanding dengan nilainya. Versi lama memakai
// Math.max(10, ...) sehingga setiap batang minimal 10% tinggi grafik:
// Realisasi AI 0,34 dari RKAP 10,89 (3%) tampil 3x lebih besar, dan nilai
// yang belum ada ("-") tetap tergambar sebagai batang. Label nilai berada
// di atas batang (CSS top: -21px), jadi batang pendek tidak perlu ditinggikan.
function overviewChartBar(tone, value, max) {
  const amount = Number(value);
  if (!Number.isFinite(amount) || amount <= 0 || !(max > 0)) {
    return `<b class="${tone} is-empty" style="height:0"><span>n/a</span></b>`;
  }
  const height = (amount / max) * 100;
  return `<b class="${tone}" style="height:max(2px, ${height.toFixed(2)}%)"><span>${overviewDecimal(amount, 2)}</span></b>`;
}
...
function parseDashboardTargetDate(value) {
  const match = String(value || "").trim().match(/^(\d{1,2})\s+([A-Za-z]+)\s+(20\d{2})$/i);
...
function overdueDays(value, reference = new Date()) {
  const target = parseDashboardTargetDate(value);
  if (!target) return 0;
...
function prioritizedOpenCr(rows = crData, reference = new Date()) {
  return [...rows]
    .filter((row) => row.status !== "Selesai")
...
function updatePerformanceStatusPanel() {
  ...
  if (scoreElement) scoreElement.textContent = measured ? nkoLabel(score) : "-";
  ...
  [scoreCard, scoreRing, scoreElement, mainStatus].forEach((element) => {
    if (!element) return;
    element.classList.remove("status-tone-green", "status-tone-amber", "status-tone-red", "status-tone-neutral");
    element.classList.add(`status-tone-${scoreStatus.tone}`);
  });
  if (indicatorCard) {
    setSafeHtml(indicatorCard, `<strong>${summary.total}</strong><span>Indikator</span>`);
  }
  if (metaIndicator) metaIndicator.textContent = `${summary.total} Indikator`;
  if (message) {
    const sourceDescription = measured ? performanceSourceDescription() : "";
    message.textContent = measured
      ? sourceDescription
      : "Periode ini belum memiliki pengukuran NKO lengkap; status tercapai tidak diberikan.";
    message.hidden = measured && !sourceDescription;
  }
  ...
  function openDetail(type) {
    try {
      ...
      if (type === "policy") {
        ...
        setSafeHtml(body, renderDetailPolicy());
        ...
        setSafeHtml(body, renderDetailBusiness());
      } else {
        ...
        setSafeHtml(body, renderDetailCr());
      }
...
function numberFromImport(value, fallback = 0) {
  ...
  if (!numericText || !/[0-9]/.test(numericText)) return fallback;
...
function importStrategySummarySheet(workbook) {
  ...
  rows.forEach((row) => {
    ...
    const validatedNumericValue = numberFromImport(validationValue, NaN);
    const value = Number.isFinite(validatedNumericValue) ? validationValue : formulaOrValue;
    const numericValue = numberFromImport(value, NaN);
...
function applyStrategyWorkbook(workbook) {
  ...
  applyAuthoritativePerformancePeriods();
  refreshPerformanceMetricsByPeriodFromRows();
...
// Polling real-time: setiap menit dashboard hanya perlu tahu apakah snapshot
// berubah. Sebelumnya setiap polling mengunduh ulang file 164 KB penuh
// (cache: "no-store" + penanda waktu) lalu mem-parse ulang seluruh workbook
// dan me-render ulang, walau datanya sama — ~236 MB/hari per tab terbuka.
// Sekarang polling memakai HEAD (hanya header, tetap melewati cache CDN
...
async function probeStrategySnapshotVersion() {
  const sourceUrl = STRATEGY_GOOGLE_XLSX_URLS[0];
  if (!sourceUrl) return null;
  const separator = sourceUrl.includes("?") ? "&" : "?";
  try {
    const response = await fetch(`${sourceUrl}${separator}_=${Date.now()}`, {
      method: "HEAD",
      cache: "no-store",
      credentials: "omit"
    });
    if (!response.ok) return null;
    return snapshotVersionFromHeaders(response.headers);
  } catch (error) {
    return null;
  }
}
...
async function refreshOnlineDashboardData(options = {}) {
  ...
  try {
    if (!force && strategySnapshotVersion) {
      const latestVersion = await probeStrategySnapshotVersion();
      strategySnapshotCheckedAt = new Date();
      // Versi sama = data tidak berubah: tidak perlu unduh, parse, atau render.
      // Jika probe gagal (null), lanjut ke unduhan penuh seperti sebelumnya.
      if (latestVersion && latestVersion === strategySnapshotVersion) {
...
function investmentKeyFromLabel(label) {
  ...
  // Kolom "Kode" di template ekspor berisi key camelCase (mis. akiRealizationPct).
  // Normalisasi di atas mengecilkan huruf, jadi key harus dicocokkan dengan
  // bentuk yang sama; sebelumnya import template sendiri memperbarui 0 nilai.
  const keyMap = Object.fromEntries(
    Object.keys(investmentFieldLabels).map((key) => [key.toLowerCase(), key])
  );
  return keyMap[normalized] || labelMap[normalized];
}
...
function renderExecutiveOverview() {
  ...
  const measuredBusinessExcellence = businessExcellenceData
    .map((row) => numberFromImport(row.realization, NaN))
    .filter((value) => Number.isFinite(value));
  const beScore = measuredBusinessExcellence.length
    ? measuredBusinessExcellence.reduce((sum, value) => sum + (Math.abs(value) <= 2 ? value * 100 : value), 0) / measuredBusinessExcellence.length
    : NaN;
  ...
  const performanceRedRows = performanceMainRows().filter((row) => performanceStatusBucket(row) === "red");
  const performanceGrayRows = performanceMainRows().filter((row) => performanceStatusBucket(row) === "gray");
  const performanceAction = performanceMeasured
    ? `Pulihkan ${performanceRedRows[0]?.indicator || `${performanceSummary.red} indikator di bawah target`} dan tuntaskan pengukuran ${performanceGrayRows[0]?.indicator || `${performanceSummary.gray} indikator tertunda`} sebelum status NKO difinalkan.`
    : "Lengkapi nilai, status, dan evidence NKO periode aktif sebelum evaluasi manajemen.";
  ...
  const crPriorityRows = prioritizedOpenCr(crData).slice(0, 2).map((row) => {
    const overdue = overdueDays(row.target);
    return { ...row, status: overdue ? `${row.status} · terlambat ${overdue} hari` : `${row.status} · target ${row.target || "belum tersedia"}` };
  });
  ...
  // Lebar meter = nilai sebenarnya. Batas bawah 4% lama membuat 0% tampil
  // sebagai isian dan nilai kecil tampak hingga beberapa kali lebih besar.
  ...
  const chartRows = [
    ...
    ["AKI", trillion(investmentData.akiTotal), trillion(investmentData.akiRealization), 0],
  ...
  setSafeHtml(target, `
    ...
    <section class="ovx-hero">
      <div class="ovx-hero-main">
        ...
        <div class="ovx-hero-tags">
          ...
          <span><i data-lucide="clock-alert"></i> Kesegaran: Strategi ${strategyDataFreshness().label} · Investasi ${investmentFreshness.label} · AO Korporat ${aoCorporateFreshness.label} · AO KP ${aoOfficeFreshness.label}</span>
        </div>
      </div>
    ...
    <section class="ovx-kpi-grid">
      ...
      ${kpiCard({ icon: "gauge", title: "NKO Strategi & Evaluasi", value: performanceMeasured ? nkoLabel(nko) : "Belum diukur", label: performanceMeasured ? `${performanceHealth.label} · ${performanceSummary.green} tercapai · ${performanceSummary.gray} belum diukur` : "Pengukuran periode belum lengkap", tone: performanceMeasured ? performanceHealth.tone : "neutral", meta: performanceMeasured ? "Target >= 100; status final menunggu cakupan lengkap" : "Tidak dinilai sebagai tercapai" })}
    ...
    <main class="ovx-grid">
      <article class="ovx-panel ovx-strategy">
        ...
        <div class="ovx-pill-row">
          ...
          ${metricPill("Tidak Ratifikasi (perlu validasi pengecualian)", policy.noRatification, "neutral")}
        </div>
        ${progressRow("Progress Change Request", `${overviewDecimal(crProgress, 2)}%`, crProgress, "blue")}
        ${progressRow("NKO Strategi & Evaluasi", performanceMeasured ? nkoLabel(nko) : "Belum diukur", performanceMeasured ? Math.min(nko, 100) : 0, performanceMeasured ? performanceHealth.tone : "neutral")}
        ${progressRow("PLN Business Excellence", Number.isFinite(beScore) ? `${overviewDecimal(beScore, 2)}% · ${beCoverage}` : "Belum diukur", Number.isFinite(beScore) ? Math.min(beScore, 100) : 0, Number.isFinite(beScore) ? "teal" : "neutral")}
      ...
      <article class="ovx-panel ovx-chart-panel">
        <div class="ovx-panel-head">
          <div><span>Perbandingan</span><h3>RKAP, Realisasi & Proyeksi Tersedia</h3></div>
          ...
          ${chartRows.map((row) => `
            <div class="ovx-chart-group">
              <div class="ovx-chart-bars">
                ${overviewChartBar("blue", row[1], chartMax)}
                ${overviewChartBar("green", row[2], chartMax)}
                ${overviewChartBar("orange", row[3], chartMax)}
              </div>
              <strong>${row[0]}</strong>
            </div>
          `).join("")}
        </div>
        <div class="ovx-chart-legend"><span><i class="blue"></i>RKAP</span><span><i class="green"></i>Realisasi</span><span><i class="orange"></i>Proyeksi sumber (AO saja)</span></div>
      </article>
      ...
      <article class="ovx-panel ovx-action-panel">
        ...
        <ol class="ovx-action-list">
          ...
          <li><b>5</b><span>${performanceAction}</span></li>
        </ol>
      </article>
    </main>
  `);
...
function evUnitCoordinateIsApproximate(item) {
  const lat = numberFromImport(item?.unitLat, NaN);
  const lng = numberFromImport(item?.unitLng, NaN);
  return !evIsPlausibleIndonesiaLatLng(lat, lng);
}
...
function initEvGeoMap() {
  ...
  if (!window.L) {
    function selectStatic(index) {
      ...
      setSafeHtml(mapEl, renderEvStaticMap(index));
    }
  ...
  const activateStaticFallback = () => {
    ...
    const selectStatic = (nextIndex) => {
      ...
      renderEvMapDetail(item);
      setSafeHtml(mapEl, renderEvStaticMap(nextIndex));
    };
    bindEvUnitList(selectStatic);
    bindEvUnitSearch(selectStatic);
    evGeoMapState = { select: selectStatic, container: mapEl };
  ...
  const unitMarkers = evGeoPriorityUnits.map((row, rowIndex) => {
    const marker = L.marker(evUnitLatLng(row), {
      icon: L.divIcon({
        className: `ev-unit-office-div-icon${evUnitCoordinateIsApproximate(row) ? " is-approximate" : ""}`,
